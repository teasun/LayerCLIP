...
def perform(process_id, dataset_list, args, model, bg_text_features, fg_text_features, cam, split_label_list):
    ...
    for im_idx, im in enumerate(tqdm(databin)):
        ...
        for image in ms_imgs:
            ...
            image_features_list, attn_weight_list = model.encode_image(image, h, w)

            #######################################
            ##########Dynamic Hierarchical#########
            #########################################
            image_features = torch.stack(image_features_list[n:],dim=0)
            ...
            for idx, label in enumerate(label_list):
                ...
                input_tensor1 = [image_features_list[-2], text_features_temp.to(device_id), h, w]
                grayscale_cam_1, logits_per_image_1, attn_weight_last_1 = cam(input_tensor=input_tensor1,
                                                                              targets=targets,
                                                                              target_size=None)  # (ori_width, ori_height))

                forw = (grayscale_cam - grayscale_cam_1)
                forw_sum = np.sum(forw)

                back = grayscale_cam_1 - grayscale_cam
                back_sum = np.sum(back)
                if forw_sum < back_sum:
                    Dec_cam = grayscale_cam
                else:
                    Dec_cam = grayscale_cam_1
                ...
                #################################################
                Dec_cam = Dec_cam[0, :]
                grayscale_cam = Dec_cam
                attn_fts = torch.from_numpy(Dec_cam)
                attn_fts = attn_fts.unsqueeze(0)
                attn_fts = attn_fts.unsqueeze(0)
                f_b, f_c, f_h, f_w = attn_fts.shape
                attn_fts_flatten = attn_fts.reshape(f_b, f_c, f_h * f_w)
                Attn_d = attn_fts_flatten.transpose(2, 1).bmm(attn_fts_flatten)
                Attn_d = torch.sigmoid(Attn_d)
                Attn_d = Attn_d.squeeze(0).cuda()
                ...
                if idx == 0:
                    ...
                    attn_weight = torch.cat(attn_weight, dim=0)[-8:]

                    attn_diff = Attn_d - attn_weight
                    ...
                    attn_weight = attn_weight * Attn_d.squeeze(0).detach()

                attn_weight = attn_weight.float().cpu()
